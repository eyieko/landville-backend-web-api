...
class PasswordResetTest(PasswordResetTokenTestBase):
    ...
    def test_user_gets_reset_token(self): 
        ...
        self.assertEqual(response.data.get('data')['message'], 'If you have an account with us we have sent an email to reset your password')
    ...
    def test_users_can_reset_passwords_successfully(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data.get('data')['message'], 'password has been changed successfully')
    ...
    def test_error_when_user_passes_invalid_token(self):
        ...
        self.assertEqual(str(response.data['token'][0]), 'This token is no longer valid, please get a new one')
    ...
    def test_error_when_user_passes_expired_token(self):
        ...
        self.assertEqual(str(response.data['detail']),'Your token has expired. Make a new token and try again')
    ...
    def test_error_when_user_passes_a_token_with_wrong_format(self):
        ...
        response = self.client.patch(
            url, self.valid_passwords, format="json")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(str(response.data['token'][0]), 'Error. Could not decode token!')
    ...
    def test_error_when_token_belonging_to_non_existent_user_is_passed(self):
        ...
        self.assertEqual(str(response.data['detail']), 'User matching this token was not found.')
    ...
    def test_error_when_non_existent_token_passed(self):
        ...
        response = self.client.patch(
            url, self.unmatching_passwords, format="json")
        self.assertEqual(str(response.data['token'][0]),"We couldn't find such token in our database")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
