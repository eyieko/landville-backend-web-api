...
class TestClientAccountDetailsViews(BaseTest):
    def test_user_must_have_client_to_post_account_details(self):
        """
        Test if client has company before submitting details
        """
        view = ClientAccountAPIView.as_view()
        self.user3.role = 'CA'
        self.user3.save()
        request = self.factory.post(ACCOUNT_DETAIL_URL,
                                    self.acc_details3,
                                    format='json')
    ...
    def test_staff_can_get_even_deleted_account_details(self):
        ...
        self.factory.delete(single_detail_url(account_number), format='json')
        request3 = self.factory.get(single_detail_url(account_number),
                                    format='json')
        self.user2.role = 'LA'
        self.user2.save()
    ...
    def test_staff_gets_the_correct_message_no_details_posted(self):
        ...
        force_authenticate(request, user=self.user3)
        resp = view(request)
        self.assertIn('There are no details posted by any client so far',
                      str(resp.data))
    ...
    def test_client_admin_gets_the_appropriate_message_no_records_posted(self):
        ...
        request2 = self.factory.get(ACCOUNT_DETAIL_URL, format='json')
        force_authenticate(request2, user=self.user1)
    ...
    def test_get_one_account(self):
        ...
        request2 = self.factory.get(single_detail_url(account_number),
                                    format='json')
    ...
    def test_delete_account_details(self):
        ...
        request2 = self.factory.delete(single_detail_url(account_number),
                                       format='json')
        force_authenticate(request2, user=self.user1)
    ...
    def test_update_account_details(self):
        ...
        request2 = self.factory.put(single_detail_url(account_number),
                                    self.account_details,
                                    format='json')
        force_authenticate(request2, user=self.user1)
    ...
    def test_post_details_no_permission(self):
        ...
        self.assertIn('Please log in to proceed.', str(result))
    ...
    def test_account_number_must_be_unique(self):
        ...
        request2 = self.factory.post(ACCOUNT_DETAIL_URL,
                                     self.account_details,
                                     format='json')
        force_authenticate(request2, user=self.user1)
