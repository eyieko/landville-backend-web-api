...
class EmailHelper:
    ...
    @staticmethod
    def send_an_email(data, user_id=None, from_approval=None):
        """Send out a an email depending on the supplied data."""
        if from_approval:
            ...
            url = f"http://{get_current_site(data[0]).domain}/api/v1/auth/verify/?token={encoded_jwt}~{user_id}"  # noqa
